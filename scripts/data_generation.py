...
scale = 0.049664  # Scale factor is the dimension of the volume in 1 axis
...
# Get camera instrinsics from segmentation_camera ADF.

f1 = open("../ADF/segmentation_camera.yaml", "r")
f1_yaml = yaml.safe_load(f1)


fva = f1_yaml["segmentation_camera"]["field view angle"]
img_height = f1_yaml["segmentation_camera"]["publish image resolution"]["height"]
img_width = f1_yaml["segmentation_camera"]["publish image resolution"]["width"]
...
metadata.create_dataset("baseline", data=0.065) # TODO: Change from static to dynamic
...
def image_gen(image_msg, curr_grp, image_name):
    try:
        ...
        curr_grp.create_dataset(image_name, data=np.asarray(cv2_img))
...
def object_info(curr_grp):
    ...
    for name, obj in objects.items():
        ...
        pos_temp = np.asarray(pos_temp) * scale
        object_grp.create_dataset("position", data=pos_temp)
        ...
        object_grp.create_dataset("quaternion", data=rot_temp)


def depth_gen(depth_msg, curr_grp):
    xyz_array = ros_numpy.point_cloud2.pointcloud2_to_array(depth_msg)
    xcol = xyz_array['x'] * scale
    ycol = xyz_array['y'] * scale
    zcol = xyz_array['z'] * scale

    scaled_depth = np.empty(np.shape(xcol), dtype=[('x', '<f4'), ('y', '<f4'), ('z', '<f4')])
    scaled_depth['x'] = xcol
    scaled_depth['y'] = ycol
    scaled_depth['z'] = zcol

    curr_grp.create_dataset("depth", data=scaled_depth)
